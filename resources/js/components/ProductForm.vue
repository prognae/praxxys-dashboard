<template>
    <form @submit="submitForm" enctype="multipart/form-data">
        <!-- only display if it is the current step -->
        <div class="step-1" v-if="currentStep == 1">
            <h3>Step 1</h3>
                <label for="name">Name <span style="color: red;">*</span></label>
                <input class="form-control" type="text" placeholder="Name" name="name" v-model="formData.name" required id="name">
        ...
        <div class="step-2" v-if="currentStep == 2">
                ...
                <button @click.prevent="updateStep(3)" :hidden="formData.images.length === 0" class="btn btn-primary">Next</button>
        </div>
...
export default {
    ...
    methods: {
        updateStep(step) { // function to update the step
        ...
        async submitForm(e) {
            ...
            try {
                // post request to create products in the database
                // sweet alerts is the alert handler used
                const responseSubmit = await axios.post('/products/create', this.formData, config);
                Swal.fire(
                            'Success!',
        ...
        validateStep1() {
            // check if all input fields are populated
